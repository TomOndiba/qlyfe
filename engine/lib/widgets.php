...
/**
 * So when a user logs in we need to make sure their widgets are matched up with
 *  our default widgets
 */
function setup_widgets_for_context($user_guid, $context, $widgets = null) {
	
	// 1. get the default widgets
	// 2. get which widgets we already have installed
	// 3. install new widgets
	// 4. relocate existing widgets
	// so... how do we do this?
	//    	we are going to use the lovely function that already exists called:
	// 		
	//		reorder_widgets_from_panel()
	//			- it expects you to pass in strings like the following for each panel:
	//				friends::36::family::0
	//			- this means the friends widget first and then the family widget and
	//				friends already exists in the db (entityid=36) and family does not exist
	//				and so needs to be created

	$default_widgets = $widgets ? $widgets : qlyfe_get_widgets($context);
	
	list ( $left, $middle, $right ) = split ( '%%', $default_widgets);
	...
	// we'll be passing these to reorder_widgets_from_panel
	$panelstring1 = "";
	$panelstring2 = "";
	$panelstring3 = "";
	...
	if ($area1widgets) {
		$panelstring1 = generate_panel_string(get_widgets($user_guid, $context, 1), $area1widgets);
	}
	if ($area2widgets) {
		$panelstring2 = generate_panel_string(get_widgets($user_guid, $context, 2), $area2widgets);
	}
	if ($area3widgets) {
		$panelstring3 = generate_panel_string(get_widgets($user_guid, $context, 3), $area3widgets);
	}

	// actually reorder the widgets according to our default settings
	reorder_widgets_from_panel($panelstring1, $panelstring2, $panelstring3, $context, $user_guid);
	
}


/**
 * Just a helper method for defaultwidgets_login
 * @param unknown_type $user_widgets
 * @param unknown_type $areawidgets
 */
function generate_panel_string($user_widgets, $areawidgets) {
	
	$panelstring = "";
	//if (!is_array($user_widgets)) return $panelstring;
	
	// temp storage for existing widgets
	$existing_widgets = array();
	
	if (is_array($user_widgets)) {
		foreach ( $user_widgets as $i => $widget_entity ) {
			// so friends => 36
			$existing_widgets[$widget_entity->handler] = $widget_entity->guid;
		}
	}
	
	// where $widget = friends or family or memberes etc..
	foreach ( $areawidgets as $widget ) {
		if (key_exists($widget, $existing_widgets)) {
			$panelstring .= "{$widget}::{$existing_widgets[$widget]}::";
		} else {				
			$panelstring .= "{$widget}::0::";
		}
	}
	return trim($panelstring, ":");
}
...
/**
 * This might be useful for my Universal Widgets.. appears to be
 * 
	...
	foreach (array(1,2,3) as $column) {
		if ($dbwidgets = get_widgets($owner,$context,$column)) {
			...
			foreach($dbwidgets as $dbwidget) {
				...
				if (($guids[1] != null && in_array($dbwidget->getGUID(),$guids[1])) || 
					($guids[2] != null && in_array($dbwidget->getGUID(),$guids[2])) || 
					($guids[3] != null && in_array($dbwidget->getGUID(),$guids[3]))
				) {
