...
function system_log($object, $event) {
	...
	if ($object instanceof Loggable) {
		...
		// @todo QLYFE BAP .. check out the access_id... do we need a system_log_classifiers table?
		if (!isset($logcache[$time][$object_id][$event])) {
			insert_data("INSERT DELAYED into {$CONFIG->dbprefix}system_log (object_id, object_class, object_type, object_subtype, event, performed_by_guid, owner_guid, access_id, enabled, time_created) VALUES ('$object_id','$object_class','$object_type', '$object_subtype', '$event',$performed_by, $owner_guid, 2, '$enabled', '$time')");
