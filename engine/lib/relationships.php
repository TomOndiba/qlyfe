 ...
 * @param int $guid_two
 * @param string $classifier (like friends/bf) ... default to null
 */
function add_entity_relationship($guid_one, $relationship, $guid_two, $clist= null) {
	global $CONFIG;
	...
	if ($clist && check_classified_relationship($guid_one, $guid_two, $clist)) {
		return false;
	} else if (!$clist && check_entity_relationship($guid_one, $relationship, $guid_two)) {
		return false;
	}
	
	$result = null;
	if ($clist) {
		// if it's a string then convert it to an object
		if (!($clist instanceof Qlyfe_CList))
			$clist = new Qlyfe_CList($clist);
		
		// foreach classifier in our list create a separate relationship
		foreach ($clist->list as $classifier)
		{ 
			if ($classifier->classifier) {
				$result = insert_data("INSERT into {$CONFIG->dbprefix}entity_relationships (guid_one, relationship, guid_two, network, classifier) values ($guid_one, '$relationship', $guid_two, '{$classifier->network}', '{$classifier->classifier}')");
			} else {
				$result = insert_data("INSERT into {$CONFIG->dbprefix}entity_relationships (guid_one, relationship, guid_two, network) values ($guid_one, '$relationship', $guid_two, '{$classifier->network}')");
			}
		}
	} else {
...
/**
 * Determine if these two things are related to each other with a classification
 * Enter description here ...
 * @param unknown_type $guid_one
 * @param unknown_type $guid_two
 * @param unknown_type $clist
 */
function check_classified_relationship($guid_one, $guid_two, $clist = null) {
	global $CONFIG;
	...
	if ($clist) {
		$clist = (($clist instanceof Qlyfe_CList) ? $clist : new Qlyfe_CList($clist));
		
		foreach ($clist->list as $cls) {
			$network = sanitise_string($cls->network);
			$classifier = sanitise_string($cls->classifier);			
			
			if ($classifier) {
				if ($row = get_data_row("SELECT * FROM {$CONFIG->dbprefix}entity_relationships WHERE guid_one=$guid_one AND guid_two=$guid_two AND network='$network' AND classifier='$classifier' limit 1"))
					return $row;
			} else {
				if ($row = get_data_row("SELECT * FROM {$CONFIG->dbprefix}entity_relationships WHERE guid_one=$guid_one AND guid_two=$guid_two AND network='$network' limit 1"))
					return $row;
			}
		}
	} else {
		if ($row = get_data_row("SELECT * FROM {$CONFIG->dbprefix}entity_relationships WHERE guid_one=$guid_one AND guid_two=$guid_two AND network is not null limit 1")) {
			return $row;
		}
	}
 ...
 * @param int $guid_two
 */
function remove_classified_relationships($guid_one, $guid_two, $clist) {
	global $CONFIG;
	...
	if ($clist)	{
		...
		foreach ($clist->list as $cls) {
			...
			if ($cls->classifier)
				delete_data("DELETE from {$CONFIG->dbprefix}entity_relationships where guid_one=$guid_one and network='$network' and classifier='$classifier' and guid_two=$guid_two");
			else
				delete_data("DELETE from {$CONFIG->dbprefix}entity_relationships where guid_one=$guid_one and network='$network' and guid_two=$guid_two");
		}
	} else {
		// there is no classifier so delete ALL classified relationships
		delete_data("DELETE from {$CONFIG->dbprefix}entity_relationships where guid_one=$guid_one and network is not null and guid_two=$guid_two");
	}
	return true;
}
 ...
 * @return true|false Depending on suc
 * cess
 */
...
function elgg_get_entities_from_relationship($options) {
	...
	$clauses = elgg_get_entity_relationship_where_sql('e', $options['relationship'],
		$options['relationship_guid'], $options['inverse_relationship'], $options['clist']);
...
function elgg_clist_to_where_clause($db_prefix = "", $clist)
{
	if ($clist_str instanceof Qlyfe_CList)
		$clist = $clist_str;
	else
		$clist = new Qlyfe_CList($clist_str);
		
	if ($clist->empty) return "";
		
	$ors = array();	
	foreach ($clist->list as $network_classifier)
	{
		
	}
		// and r.network = 'network' and r.classification = 'classification' or ...	
}
 ...
 * @param $clist
 * @param $just_classified set to true if you want to get just classified relationships
 * @return mixed
 */
function elgg_get_entity_relationship_where_sql($table, $relationship = NULL, $relationship_guid = NULL, $inverse_relationship = FALSE, $clist = NULL, $just_classified = FALSE) {
	...
	// here is where we put our classifiers if they exist
	$clist = ($clist instanceof Qlyfe_CList) ? $clist : new Qlyfe_CList($clist);
	if (!$relationship && ($clist->isEmpty() || $clist->__toString() == "public")) {
		$wheres[] = "r.network is not null";
	}
	else if ($clist) {

		if ($clist->list && sizeof($clist->list) > 0) {
			$clist_where = array();
			foreach ($clist->list as $c) {
				if ($c->classifier)
					$clist_where[] = "(r.network = '{$c->network}' and r.classifier = '{$c->classifier}')";
				else
					$clist_where[] = "(r.network = '{$c->network}')";
			}
			$wheres = array_merge($wheres, array(implode(" OR ", $clist_where)));
		}
	}
 ...
 */
function list_entities_from_relationship($relationship, $relationship_guid, $inverse_relationship = false, $type = ELGG_ENTITIES_ANY_VALUE, $subtype = ELGG_ENTITIES_ANY_VALUE, $owner_guid = 0, $limit = 10, $fullview = true, $viewtypetoggle = false, $pagination = true, $clist = null) {
	$limit = (int) $limit;
...
function get_entities_by_relationship_count($relationship, $inverse_relationship = true, $type = "", $subtype = "", $owner_guid = 0, $limit = 10, $offset = 0, $count = false, $site_guid = 0) {
	...
	$query .= get_entity_access_sql_suffix("e"); // Add access controls
...
function relationship_notification_hook($event, $object_type, $object) {
	...
	)
	{
		...
		$network = $object->network;// the network this relationship is on
		$relationship = $object->relationship;
		if ($network) {
			// Notify target user
			return notify_user($object->guid_two, $object->guid_one, sprintf(elgg_echo("$network:connect:notification:subject"), $user_one->name),
				sprintf(elgg_echo("$network:connect:notification:body"), $user_one->name, $user_one->getURL())
			);
		}
	}
}
...
register_elgg_event_handler('create','relationship','relationship_notification_hook');
