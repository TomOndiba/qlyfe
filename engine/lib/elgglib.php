...
/**
 * Forwards to the users home page.
 * @return nothing|false
 */
function forward_home() {
		if (is_plugin_enabled('home'))
			return forward('pg/home');
		else
			return forward('pg/dashboard');
}		
 ...
 * @see list_user_connection_objects
 ...
 * Things that extend Relatable can be connected to each other via a entity_relationship
 *  or a connection
 */
interface Relatable {
	/**
	 ...
	 */
	public function connectTo($friend_guid, $relationship = "", $clist = null);

	/**
	 * Removes all our connections to this object
	 *
	 * @param int $friend_guid The GUID of the user to remove
	 * @param classifier list $clist optional if you only want to disconnect some of the relationships
	 * 	like any good clist you can pass it just the network name (ie friends and it will work)
	 */
	public function disconnectFrom($friend_guid, $clist = null);

	/**
	 * Determines whether or not the current user is connected to this entity
	 * (from the current users point of view)
	 */
	public function isConnected();

	/**
	 * Determines whether or not this entity is connected to another entity
	 * @param int $user_guid The GUID of the entity this entity may or may not be connected to
	 */
	public function isConnectedTo($user_guid);

	/**
	 * Determines whether or not a foreign entity has connected to me
	 * @param int $user_guid The GUID of the foreign entity
	 */
	public function isConnectedFrom($user_guid);

	/**
	 * Returns all entities this entity is connected to
	 *
	 ...
	 * @param string $clist 
	 */
	public function getConnectedTo($subtype = "", $limit = 10, $offset = 0, $clist = "");

	/**
	 * Returns entities that have connected to this entity
	 *
	 * @param string $subtype The subtype of entity to return
	 * @param int $limit The number of entities to return
	 * @param int $offset Indexing offset
	 */
	public function getConnectedFrom($subtype = "", $limit = 10, $offset = 0);
	...
	/**
	 * Returns objects in the containers of this entity's connections
	 *
	 * @param string $subtype The subtype of entity to return
	 * @param int $limit The number of entities to return
	 * @param int $offset Indexing offset
	 */
	public function getConnectedObjects($subtype = "", $limit = 10, $offset = 0);
...
class Qlyfe_Classifier {
	public $network = null;
	public $classifier = null;
	
	public function __construct($str) {
		$nc_array = explode("/", $str);
		if (empty($nc_array)) throw new Exception("empty classifier");
		
		$this->network = $nc_array[0];
		if (sizeof($nc_array) == 2)
			$this->classifier = $nc_array[1];		
	}
	
	public function __toString() {
		if ($this->classifier)
			return $this->network . "/" . $this->classifier;
		else
			return $this->network;
	}
}
 ...
 *  
 * by encapsulating it as a class this way you can still assign it as an entity/metadata/annotation
 *  attribute eg. $entity->access_id = $some_other_access_id (access_id is now a Qlyfe_CList object)
 *  
 *  A "CList' is a classifier list which basically means who can see this particular piece of information
 *   for example $entity->access_id = new Qlyfe_CList(array("friends/bf", "family/moms_side"))
 *   
 *  If it's public or private this should override all other classifications. 
 *  Private is implied by all classifications including an empty classification
 */
class Qlyfe_CList {
	
	
	const _PUBLIC = "public"; // everyone can see it
	const _PRIVATE = "private"; // only the owner can see it
	...
	/**
	 * @param array $array an array of classifiers, can also take a comma separated list
	 */
	public function __construct($array = null) {
		if (!$array) {
			$this->list = array();
		} else if ($array instanceof Qlyfe_Classifier) {
			$this->list = array(); $this->list[] = $array;
		} else if (!is_array($array)) {
			$this->list = explode(",", $array);
		} else {
			$this->list = $array;
		}
		// make them into classifiers
		for ($i = 0; $i < sizeof($this->list); $i++)
		{
			if (!($this->list[$i] instanceof Qlyfe_Classifier))
				$this->list[$i] = new Qlyfe_Classifier($this->list[$i]);
		}
	}

	/**
	 * Adds this classifier to our list if it doesn't already exist
	 * @param unknown_type $classifier
	 */
	public function add($classifier) {
		if (!($classifier instanceof Qlyfe_Classifier))
			$classifier = new Qlyfe_Classifier($classifier);
			
		if (!in_array($classifier, $this->list))
			$this->list[] = $classifier;
	}
	...
	/**
	 * @return this as a string of classifier1,classifier2,... etc
	 */
	public function __toString()
	{
		//if (sizeof($this->list) == 0) return self::_PRIVATE;
		$str = "";
		foreach ($this->list as $classifier) {
			$str .= $classifier . ",";
		}
		return trim($str, ",");
	}
...
function js_friendly_classifier($classifier) {
	return str_replace("/","_", str_replace("'","_", str_replace(" ","_",$classifier)));
}
 ...
 *
 */
define('ACCESS_DEFAULT', new Qlyfe_CList(array(Qlyfe_CList::_PUBLIC)));//-1
define('ACCESS_PRIVATE', new Qlyfe_CList(array(Qlyfe_CList::_PRIVATE))); // 0
define('ACCESS_LOGGED_IN', new Qlyfe_CList(array(Qlyfe_CList::_PUBLIC)));//1
define('ACCESS_PUBLIC', new Qlyfe_CList(array(Qlyfe_CList::_PUBLIC))); // 2
//define('ACCESS_FRIENDS', new Qlyfe_CList(array(Qlyfe_CList::_FRIENDS_BF,Qlyfe_CList::_FRIENDS_F))); // -2
