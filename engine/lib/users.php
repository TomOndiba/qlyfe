...
class ElggUser extends ElggEntity
	...
	protected function initialise_attributes() {
		...
		$this->attributes['nickname'] = array();		
	}
	...
	function __construct($guid = null) {
		...
		if (!empty($guid)) {
			...
			if(isset($_SESSION['user']))
				$this->attributes['name'] = $this->getName($_SESSION['user']->get_user_relationship($this));				
			else
	...
	public function getDefaultName(){
		$metadata1 = get_metadata_byname($this->getGUID(),"firstname");
		$metadata2 = get_metadata_byname($this->getGUID(),"lastname");
		if($metadata1)
			$firstname = $metadata1->value;
		if($metadata2)	
			$lastname = $metadata2->value;
	...
	/**
	 * get Nick Name based on network and classifier
	 *
	 * @return bool
	 */

	public function getName($network='public',$classifier=""){
		if(count($this->attributes['nickname']) <= 0)
			$this->getNickName();

		//print_r($this->attributes['nickname']);			
		if($classifier != "")
			$name =  $this->attributes['nickname'][$network]['classifier'][$classifier];		
		else
			$name =  $this->attributes['nickname'][$network];
	...
	public function getNickName(){
		$metadata = get_metadata_byname($this->getGUID(),"nickname");	
		$narr = array();

		$fullname = $this->getDefaultName();
		...
		if (is_array($metadata)) {
			foreach($metadata as $md) {
				...
				if($c1 != ""){
					$narr[$network][$c1] = $md->value;
				}else{
					$narr[$network] = $md->value;									
				}	
			}	
		} else {
				if($metadata->value == "")
					$metadata->value = $fullname;

				$clist = $metadata->access_id;
				if ($clist && $clist->list) {
					foreach ($clist->list as $classifier) {
						//$c1 = $classifier->classifier;
						$network = $classifier->network;	
				...
				if($network != ""){
				if($c1 != ""){
					$narr[$network]['classifier'][$c1] = $metadata->value;
				}else{
					$narr[$network] = $metadata->value;									
				}	
				}
		}
		if(count($narr) <= 0){			
			$narr['public'] =  $fullname;
		}		
		$this->attributes['nickname'] = $narr;
	}
	...
	/**
	 * Make a connection between these two guids
	 * @todo QLYFE BAP add the relationship type while checking eligability
	 * @param $guid
	 * @param $relationship
	 * @param $clist
	 */
	function connectTo($guid, $relationship = "", $clist = null) {
		if (!$relationship) throw new Exception("empty relationship in user connect");
		if (!eligable_for_relationship($this->getGUID(), $guid))
			return false;
		return add_entity_relationship($this->getGUID(), $relationship, $guid, $clist);
	}
	
	/**
	 * Disconnect from this user
	 * @param int $friend_guid The GUID of the user to remove
	 * @param string $clist an optionial classifier list in case of only a partial disconnect
	 * @return true|false Depending on success
	 */
	function disconnectFrom($friend_guid, $clist = null) {
		
		$user_guid = (int) $this->getGUID();
		...
		*/
		...
		return remove_classified_relationships($user_guid, $friend_guid, $clist);
		//return remove_entity_relationship($user_guid, "friend", $friend_guid);
	}


	function get_user_relationship($user2){
		$firstguid = (int) $this->getGUID();
		$secondguid = (int) $user2->getGUID();
		if($firstguid == $secondguid){
			$network = "public";
		}else{
			if($records = user_is_connected($secondguid,$firstguid)){
	...
	function isConnected() {
		return user_is_connected(get_loggedin_userid(), $this->getGUID());
	}

	/**
	 * Determines whether this user is connected to another user
	 *
	 * @param int $user_guid The GUID of the user to check 
	 * @return true|false
	 */
	function isConnectedTo($user_guid) {
		return user_is_connected($this->getGUID(), $user_guid);
	}

	/**
	 * Determines whether or not this user is connected to FROM another user
	 *
	 * @param int $user_guid The GUID of the user to check against
	 * @return true|false
	 */
	function isConnectedFrom($user_guid) {
		return user_is_connected($user_guid, $this->getGUID());
	}

	/**
	 * Retrieves a list of everyone this user is connected to 
	 *
	 * @param string $subtype Optionally, the subtype of user to filter to (leave blank for all)
	 * @param int $limit The number of users to retrieve
	 * @param int $offset Indexing offset, if any
	 * @param string $clist (or Qlyfe_CList)
	 * @return array|false Array of ElggUsers, or false, depending on success
	 */
	function getConnectedTo($subtype = "", $limit = 10, $offset = 0, $clist = null) {
		return get_user_connections($this->getGUID(), $clist, $subtype, $limit, $offset);
	}

	/**
	 * Retrieves a list of people who have connected to this user
	 *
	 * @param string $subtype Optionally, the subtype of user to filter to (leave blank for all)
	 * @param int $limit The number of users to retrieve
	 * @param int $offset Indexing offset, if any
	 * @return array|false Array of ElggUsers, or false, depending on success
	 */
	function getConnectedFrom($subtype = "", $limit = 10, $offset = 0) {
		return get_user_connected_from($this->getGUID(), $subtype, $limit, $offset);
	}
	...
	/**
	 * Get an array of ElggObjects owned by this user's connections
	 *
	 * @param string $subtype The subtype of the objects, if any
	 * @param int $limit Number of results to return
	 * @param int $offset Any indexing offset
	 * @param string $clist (bf, f, a or empty)
	 */
	public function getConnectedObjects($subtype = "", $limit = 10, $offset = 0, $clist = "") {
		return get_user_connected_objects($this->getGUID(), $subtype, $limit, $offset, $clist);
	}
	...
	public function __toString() {
		return parent::__toString() . " username=" . $this->username;
	}
}
...
function create_user_entity($guid, $name, $username, $password, $salt, $email, $language, $code) {
	...
	if ($row) {
		...
		if ($exists = get_data_row("SELECT guid from {$CONFIG->dbprefix}users_entity where guid = {$guid}")) {
			...
			// @qlyfe addition... the user is the only object that owns itself (for now)
			$result = update_data("UPDATE {$CONFIG->dbprefix}entities set owner_guid={$guid} where guid = {$guid}");
...
/**
 * Determine if these are both valid users and we're not trying to connect to ourself
 * @param int $user_guid
 * @param int $friend_guid
 */
function eligable_for_relationship($user_guid, $friend_guid) {
	$user_guid = (int) $user_guid;
	$friend_guid = (int) $friend_guid;
...
/**
 * Determines whether or not a user is connected to another user
 *
 * @param int $user_guid The GUID of the user
 * @param int $friend_guid The GUID of the friend
 * @return true|false
 */
function user_is_connected($user_guid, $friend_guid) {
	//return check_entity_relationship($user_guid, "friend", $friend_guid);
	return check_classified_relationship($user_guid, $friend_guid);
}
 ...
 */
function get_user_connections($user_guid, $clist = null, $subtype = ELGG_ENTITIES_ANY_VALUE, $limit = 10, $offset = 0) {
	return elgg_get_entities_from_relationship(array(
		'relationship_guid' => $user_guid,
		'types' => 'user',
		'subtypes' => $subtype,
 ...
 * @return false|array Either an array of ElggUsers or false, depending on success
 */
function get_user_connected_from($user_guid, $subtype = ELGG_ENTITIES_ANY_VALUE, $limit = 10, $offset = 0) {
	return elgg_get_entities_from_relationship(array(
		'relationship_guid' => $user_guid,
 ...
 * @param int $limit The number of results to return (default 10)
 * @param int $offset Indexing offset, if any
 * @param string $clist (bf, f, a)
 * @param int $timelower The earliest time the entity can have been created. Default: all
 * @param int $timeupper The latest time the entity can have been created. Default: all
 * @return false|array An array of ElggObjects or false, depending on success
 */
function get_user_connected_objects($user_guid, $subtype = ELGG_ENTITIES_ANY_VALUE, $limit = 10, $offset = 0, $clist = "", $timelower = 0, $timeupper = 0) {
	if ($connections = get_user_connections($user_guid, $clist, "", 999999, 0)) {
		$connectionguids = array();
		foreach($connections as $connection) {
			$connectionguids[] = $connection->getGUID();
		}
...
/**
 * Counts the number of objects owned by people this user is connected to
 *
 * @param int $user_guid The GUID of the user to get the friends of
 * @param string $subtype Optionally, the subtype of objects
 * @param int $timelower The earliest time the entity can have been created. Default: all
 ...
 */
function count_user_connection_objects($user_guid, $subtype = ELGG_ENTITIES_ANY_VALUE, $timelower = 0, $timeupper = 0) {
...
/**
 * Displays a list of a user's connections' objects of a particular subtype, with navigation.
 *
 * @see elgg_view_entity_list
 *
 * @param int $user_guid The GUID of the user
 ...
 * @return string The list in a form suitable to display
 */
function list_user_connection_objects($user_guid, $subtype = "", $limit = 10, $fullview = true, $viewtypetoggle = true, $pagination = true, $timelower = 0, $timeupper = 0) {
	$offset = (int) get_input('offset');
	$limit = (int) $limit;
	$count = (int) count_user_connection_objects($user_guid, $subtype, $timelower, $timeupper);
	$entities = get_user_connection_objects($user_guid, $subtype, $limit, $offset, "", $timelower, $timeupper);
...
function get_user_by_username($username) {
	...
	$username = sanitise_string($username);
	// @todo QLYFE BAP .. had to comment this out since it wasn't letting us log in
	//    is this the best way to handle this?
	//$access = get_entity_access_sql_suffix('e');
...
function set_user_validation_status($user_guid, $status, $method = '') {
	...
	if ($status) {
		...
		$validated = get_metadata_byname($user_guid,  'validated', $user_guid);
		$validated_method = get_metadata_byname($user_guid,  'validated_method', $user_guid);
...
function register_user_check($username, $password, $password2, $firstname,$lastname, $email, $allow_multiple_emails = false, $friend_guid = 0, $invitecode = '') {
	...
	if ($password  != $password2) {
		throw new RegistrationException(elgg_echo('registration:passwordnotmatch'));
	}
 ...
 */
function register_user($username, $password, $password2, $firstname,$lastname, $email, $allow_multiple_emails = false, $friend_guid = 0, $invitecode = '') {
	// Load the configuration
	global $CONFIG;

	if(!register_user_check($username, $password, $password2, $firstname,$lastname, $email, $allow_multiple_emails, $friend_guid, $invitecode))
		return false;
	...
	if ($friend_guid) {
		error_log("AUTOMATIC CONNECT TO FRIEND IS NO LONGER WORKING SEE THIS ERROR!!!");
		//throw new Exception("this doesn't work anymore");
		// @todo QLYFE BAP .. so this doesn't work because the connectTo function 
		// needs a relationship and a classifier.. we can't just connect two things
		// arbitrarily anymore... to fix we can have them see the pending connections 
		// when they log in or something
		/**
		if ($friend_user = get_user($friend_guid)) {
			if ($invitecode == generate_invite_code($friend_user->username)) {
				$user->connectTo($friend_guid);
				$friend_user->connectTo($user->guid);
...
function generate_newinvite_code($username) {
...
function collections_page_handler($page_elements) {
	if (isset($page_elements[0])) {
		if ($page_elements[0] == "add") {
			...
			require_once(dirname(dirname(dirname(__FILE__))) . "/connect/add.php");
		} else {
...
function users_init() {
	...
	register_action("connect/add");
	register_action("connect/remove");
	register_action('connect/addcollection');
	register_action('connect/deletecollection');
	register_action('connect/editcollection');
