...
function is_plugin_enabled($plugin, $site_guid = 0) {
	...
	if (!$ENABLED_PLUGINS_CACHE) {
		$site = get_entity($site_guid, $site_guid);
		if (!($site instanceof ElggSite)) {
			throw new InvalidClassException(sprintf(elgg_echo('InvalidClassException:NotValidElggStar'), $site_guid, "ElggSite"));
		}
		...
		// for all of the plugins in the config file.. make sure they're enabled
		foreach ($CONFIG->enabled_plugins as $plugin) {
			if (!in_array($plugin, $enabled_plugins)) {
				enable_plugin($plugin, $site_guid);
			}
 		}
