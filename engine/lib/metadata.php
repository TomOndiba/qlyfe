...
class ElggMetadata extends ElggExtender {
	/**
	 * Construct a new ElggMetadata object, optionally from a given id value or row.
	 * @param mixed $id
	 */
	function __construct($id = null) {
		...
		if (!empty($id)) {
			...
			if ($metadata) {
				...
				$this->access_id = select_metadata_classifiers($this->id);
			}
		}
	}
...
function get_metadata($id) {
	...
	$md_access = get_metadata_access_sql_suffix("m");
 ...
 */
function create_metadata($entity_guid, $name, $value, $value_type, $owner_guid, $clist = NULL, $allow_multiple = false) {
	global $CONFIG;
	...
	if ($clist == NULL) $clist = ACCESS_PRIVATE;
	...
	if (($existing) && (!$allow_multiple) && (isset($value))) {
		$id = $existing->id;
		$result = update_metadata($id, $name, $value, $value_type, $owner_guid, $clist);
		...
		// If ok then add it
		// @qlyfe addition .. removed acccess_id
		$id = insert_data("INSERT into {$CONFIG->dbprefix}metadata (entity_guid, name_id, value_id, value_type, owner_guid, time_created) VALUES ($entity_guid, '$name','$value','$value_type', $owner_guid, $time)");

		// delete the existing
		delete_metadata_classifiers($id);
		// insert the new ones
		insert_metadata_classifiers($id, $clist);
 ...
 * @param Qlyfe_CList string or array $clist
 */
function update_metadata($id, $name, $value, $value_type, $owner_guid, $clist) {
	global $CONFIG;
...
*/
	...
	$access = get_metadata_access_sql_suffix();
	...
	// If ok then add it
	$result = update_data("UPDATE {$CONFIG->dbprefix}metadata set value_id='$value', value_type='$value_type', owner_guid=$owner_guid where id=$id and name_id='$name'");
 ...
 * @param int $looker who is trying to get this metadata>
 */
function get_metadata_byname($entity_guid,  $meta_name, $looker = null) {
	global $CONFIG;
	...
	$access = get_entity_access_sql_suffix("e", $looker);
	$md_access = get_metadata_access_sql_suffix("m", $looker);
 ...
 *
 * @param int $entity_guid
 * @param int $looker The GUID of the looker
 */
function get_metadata_for_entity($entity_guid, $looker = null) {
	global $CONFIG;
...
function find_metadata($meta_name = "", $meta_value = "", $entity_type = "", $entity_subtype = "", $limit = 10, $offset = 0, $order_by = "", $site_guid = 0) {
	...
	$query .= get_metadata_access_sql_suffix("e"); // Add access controls
	$query .= ' and ' . get_metadata_access_sql_suffix("m"); // Add access controls
...
function elgg_get_entity_metadata_where_sql($e_table, $n_table, $names = NULL, $values = NULL, $pairs = NULL, $pair_operator = 'AND', $case_sensitive = TRUE, $order_by_metadata = NULL, $owner_guids = NULL) {
	...
	$access = get_metadata_access_sql_suffix('n_table');
	...
	if (is_array($pairs)) {
		...
		foreach ($pairs as $index => $pair) {
			...
			$access = get_metadata_access_sql_suffix("n_table{$i}");
...
function clear_metadata($entity_guid) {
	...
	if ($entity = get_entity($entity_guid)) {
		if ($entity->canEdit()) {
			delete_data("DELETE FROM {$CONFIG->dbprefix}classifiers where type='metadata' and id in (select m.id from {$CONFIG->dbprefix}metadata m where m.entity_guid={$entity_guid})");
