	...
	ArrayAccess // Override for array access
{
	 ...
	 */
	public function getIcon($size = 'medium',$network='') {
		if (isset($this->icon_override[$size])) {
			return $this->icon_override[$size];
		}
		return get_entity_icon_url($this, $size,$network);
	}
	...
	function __toString() {
		return "ElggEntity type={$this->type} owner={$this->owner_guid} subtype={$this->subtype} guid={$this->guid}";
	}
}
 ...
 */
function create_entity($type, $subtype, $owner_guid, $clist, $site_guid = 0, $container_guid = 0) {
	global $CONFIG;
	...
	$user = get_loggedin_user();
	if (!access_get_show_hidden_status() && !can_write_to_container($user->guid, $owner_guid, $type)) {
		return false;
	}
	if ($owner_guid != $container_guid) {
		if (!access_get_show_hidden_status() && !can_write_to_container($user->guid, $container_guid, $type)) {
			// 08/24/10 Dennis commented this out. We are no longer checking
			// if owner_guid can write to container since permissions are now handled by CList 
			//return false;
		}
	}
	if ($type=="") {
	...
	$new_id = insert_data("INSERT into {$CONFIG->dbprefix}entities
		(type, subtype, owner_guid, site_guid, container_guid, time_created, time_updated) values
		('$type',$subtype, $owner_guid, $site_guid, $container_guid, $time, $time)");
	insert_entity_classifiers($new_id, $clist);
	return $new_id;
}
 ...
 * @param int $looker The GUID of who is trying to get this entity
 */
function get_entity_as_row($guid, $looker = null) {
	global $CONFIG;
	...
	$guid = (int) $guid;
	$access = get_entity_access_sql_suffix('', $looker);
...
function entity_row_to_elggstar($row) {
	...
	$new_entity->access_id = select_entity_classifiers($new_entity->guid);
 ...
 * @param int $looker The GUID of who is looking at this entity
 * @return a child of ElggEntity appropriate for the type.
 */
function get_entity($guid, $looker = null) {
	static $newentity_cache;
	$new_entity = false;
	if ((!$newentity_cache) && (is_memcache_available())) {
		$newentity_cache = new ElggMemcache('new_entity_cache');
	...
	return entity_row_to_elggstar(get_entity_as_row($guid, $looker));
}
...
function elgg_get_entities(array $options = array()) {
	...
	// Add access controls
	$query .= get_entity_access_sql_suffix('e');
	if (!$options['count']) {
...
function get_entity_dates($type = '', $subtype = '', $container_guid = 0, $site_guid = 0, $order_by = 'time_created') {
	...
	$where[] = get_entity_access_sql_suffix();
 ...
 * @param string $size
 */
function get_entity_icon_url(ElggEntity $entity, $size = 'medium',$network='') {
	global $CONFIG;
	...
	$url = trigger_plugin_hook('entity:icon:url', $entity->getType(), array('entity' => $entity, 'viewtype' => $viewtype, 'size' => $size,'network' => $network), $url);
