 ...
 */
function add_to_river($view,$action_type,$subject_guid,$object_guid,$clist = null,$posted = 0, $annotation_id = 0) {
	...
	insert_system_log_classifiers($id, $clist);
...
function remove_from_river_by_subject($subject_guid) {
	...
	// Remove
	delete_data("delete from {$CONFIG->dbprefix}classifiers where type='river' and 
		id in (select r.id from {$CONFIG->dbprefix}river r where r.subject_guid = {$subject_guid})");
...
function remove_from_river_by_object($object_guid) {
	...
	// Remove
	delete_data("delete from {$CONFIG->dbprefix}classifiers where type='river' and 
		id in (select r.id from {$CONFIG->dbprefix}river r where r.object_guid = {$object_guid})");
...
function remove_from_river_by_annotation($annotation_id) {
	...
	// Remove
	delete_data("delete from {$CONFIG->dbprefix}classifiers where type='river' and 
		id in (select r.id from {$CONFIG->dbprefix}river r where r.annotation_id = {$annotation_id})");
 ...
 *
 * @param int $object_guid The GUID of the entity
 * @param int $clist The classification list
 * @return true|false Depending on success
 */
function update_river_access_by_object($object_guid, $clist) {
	// Sanitise
	$object_guid = (int) $object_guid;
	...
	delete_data("delete from {$CONFIG->dbprefix}classifiers where type='river' and 
		id in (select r.id from {$CONFIG->dbprefix}river r where r.object_guid = {$object_guid})");
	$row = get_data("select id from {$CONFIG->dbprefix}river where object_guid = {$object_guid}");
	if ($row) {
		insert_river_classifiers($row->id, $clist);
	}
	return true;
	// Remove
...
function get_river_items($subject_guid = 0, $object_guid = 0, $subject_relationship = '', $type = '',
	...
	$where = array();
	// @todo QLYFE BAP .. are we using the river?
	$where[] = str_replace("and enabled='yes'",'',str_replace('owner_guid','subject_guid',get_access_sql_suffix("river")));
