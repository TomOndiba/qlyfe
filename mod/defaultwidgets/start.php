...
function defaultwidgets_init() {
	...
	// BAP ok.. we need to add something that when a user logs in it checks to see if there are
	// any new widgets to install for them
	//register_elgg_event_handler('login', 'user', 'defaultwidgets_login');
 ...
 *  our default widgets
 */
function defaultwidgets_login($event, $object_type, $object) {
	...
	foreach ( $contexts as $context ) {
		...
		setup_widgets_for_context($user_guid, $context, $current_widgets);
	}	
}
	...
	if (! empty ( $guid )) {
		...
		if ($user = get_entity ( $guid )) {
			...
			if ($user->canEdit ()) {
				...
				$contexts = qlyfe_get_all_context_widgets();//array ('profile', 'dashboard' );
				...
				if (true) {//(isset ( $entities [0] )) {
					...
					foreach ( $contexts as $context ) {
						//$current_widgets = $entity->$context;
						$current_widgets = qlyfe_get_widgets($context);
