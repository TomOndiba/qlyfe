...
if ($mysql_dblink) {
	...
	if (@mysql_select_db($CONFIG->dbname,$mysql_dblink)) {
		...
		// BAP: this returns false if the devtools are turned on...
		// and I don't know what the point of it is anyway, why not load it directly always
		//if ($simplecache_enabled) {
		if (true) {

			// first try to read icon directly
			$user_path = date('Y/m/d/', $joindate) . $guid;
			$filename = $dataroot . $user_path . "/profile/" . $username .$network. $size . ".jpg";
